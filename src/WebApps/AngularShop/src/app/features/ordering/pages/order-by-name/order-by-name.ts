...
@Component({
  ...
  templateUrl: './order-by-name.html',
  styles: [`
...
export class OrderByName {
  ...
  protected search(): void {
    ...
    this.service.byName(name).subscribe({
      next: (orders) => {
      ...
      error: () => {
        this.orders.set([]);
        this.loading.set(false);
        this.error.set('No orders found for that name, or the request failed.');
      }
    });
  }
